use super::{ColorRGB, Framebuffer, TraceContext, XYCoord};
...
impl Pixel {
    ...
    fn subdivide(
        ...
        let b = *self.stash[bottom_left.0][bottom_left.1 + off].get_or_insert_with(|| {
            tracer.sample_coord(XYCoord {
                x: coord.x + adjust,
                y: coord.y,
            })
        });

        let e = *self.stash[bottom_left.0 - off][bottom_left.1 + off].get_or_insert_with(|| {
            tracer.sample_coord(XYCoord {
                x: coord.x + adjust,
                y: coord.y - adjust,
            })
        ...
        let d = *self.stash[bottom_left.0 - off][bottom_left.1].get_or_insert_with(|| {
            tracer.sample_coord(XYCoord {
                x: coord.x,
                y: coord.y - adjust,
            })
        ...
        if !samples_differ(&samples) || depth == min_depth {
            ...
            let b = self.subdivide(
                XYCoord {
                    x: coord.x + adjust,
                    y: coord.y,
                },
                (bottom_left.0, bottom_left.1 + off),
                d,
                tracer,
                min_depth,
            );

            let e = self.subdivide(
                XYCoord {
                    x: coord.x + adjust,
                    y: coord.y - adjust,
                },
                (bottom_left.0 - off, bottom_left.1 + off),
                d,
                tracer,
                min_depth,
            );

            let d = self.subdivide(
                XYCoord {
                    x: coord.x,
                    y: coord.y - adjust,
                },
                (bottom_left.0 - off, bottom_left.1),
                d,
                tracer,
                min_depth,
            );
